...
def build_photo_analysis_docs() -> list[dict]:
    """Load pre-built photo analysis docs from JSONL (generated by analyze_photos.py)."""
    jsonl_path = OUTPUT_DIR / "photo_analysis_docs.jsonl"
    if not jsonl_path.exists():
        click.echo("  photo_analysis_docs.jsonl not found, skipping (run analyze_photos.py first)")
        return []
    docs = []
    ...
    click.echo(f"  Loaded {len(docs)} photo analysis docs")
    return docs
