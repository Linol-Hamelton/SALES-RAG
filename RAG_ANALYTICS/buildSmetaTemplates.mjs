...
const SEED_DEALS = {
  ...
  // П8.1: SEED для брендинг-категорий — canonical_id выбирался из чистых
  // offer-шаблонов (не production-заказов). Root cause: picker брал outlier
  // event-заказ «ЗМ- Брендбук Инновационный колледж» (2.5M₽, 39 поз.) вместо
  // типичного дизайн-пакета (66K₽, 12 чистых поз.).
  "Брендбук": {
    dataset: "offers",
    canonical_id: "37272",  // Брендбук "Стандарт" — 12 поз., 66K — логобук+гайдлайн+макеты
    seed_ids: ["37272", "48444", "48452", "21220", "21218", "21216"],
    enrich_regex: /брендбук/i,
    enrich_exclude: /шоколад|сахар|бутылк|повербанк|значок|брелок|ежедневник|зажигалк|флешк|плед|шапк|зонт|носок|скотч|флаг|бейдж|папк|брошюр|открытк|календар|бирк|ценник/i,
  },

  // П8.1: Фирменный стиль — canonical из offer-шаблонов «Разработка фирменного стиля».
  // Текущий picker тянул deals со слабым сигналом (freq=0.08) и разрозненными
  // позициями. Seed-шаблоны 21216-21220 — чистые упакованные пакеты услуг.
  "Фирменный стиль": {
    dataset: "offers",
    canonical_id: "21218",  // Разработка фирменного стиля пакет "Базовый" — 7 поз., 51.5K
    seed_ids: ["21218", "21216", "21220"],
    enrich_regex: /фирменн\w*\s*стил/i,
    enrich_exclude: /ручк|кружк|термокружк|футболк|кепк|бейсболк|шоппер|худи|свитшот|толстовк|шоколад|сахар|бутылк|повербанк|значок|брелок|ежедневник|зажигалк|флешк|плед|шапк|зонт|носок/i,
  },

  // П8.2: Титульные вывески — canonical_id из production-заказов с чистыми
  // позициями вывески (оракал+аппликация+полиграфия). Текущий picker выбирал
  // deal 65746 («МХ_титульная вывеска»), содержащий «Шоколад с логотипом 300 шт»
  // и другие нерелевантные позиции (freq=0.01 у всех 11 поз.).
  "Титульные вывески": {
    dataset: "orders",
    canonical_id: "48137",  // ЮД-0336 — 6 поз., 14K: Титульная вывеска полиграфия + Оракал + Аппликация
    seed_ids: ["48137", "54040", "33034", "17814"],
    enrich_regex: /титульн\w*\s+вывеск|вывеск.*орг.стекл|вывеск.*компо|вывеск.*пвх/i,
    enrich_exclude: /шоколад|сахар|табличк|ручк|кружк|футболк|магнит|сувенир/i,
  },
};
